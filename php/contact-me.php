...
if($_POST) {
    ...
    $to_Email = "ahmed@nitul.net"; // Write your email here
    ...
    if(!$Mailsending) {
        ...
        $output = json_encode(array('type'=>'error', 'text' => '<i class="icon ion-close-round"></i> Oops! Looks like something went wrong, please send me an email to: ahmednitul@gmail.com'));
        die($output);
        
    } else {
        $output = json_encode(array('type'=>'message', 'text' => '<i class="icon ion-checkmark-round"></i> Hello '.$_POST["userName"] .', Your message has been sent, I will get back to you asap !'));
        die($output);
    }
}
?>